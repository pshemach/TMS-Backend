...
@geo_constraint_router.post("/", status_code=status.HTTP_201_CREATED, response_model=schemas.GeoConstraintResponse)
def create(request: schemas.GeoConstraintCreate, db: Session = Depends(get_db)):
    geo = ops.create_geo_constraint(request, db)
    return _enrich(geo)


@geo_constraint_router.get("/", response_model=List[schemas.GeoConstraintResponse])
def list_all(
...
@geo_constraint_router.get("/{id}", response_model=schemas.GeoConstraintResponse)
def get_one(id: int, db: Session = Depends(get_db)):
    geo = ops.get_geo_constraint(id, db)
    return _enrich(geo)


@geo_constraint_router.put("/{id}", response_model=schemas.GeoConstraintResponse)
